...
export default function LeadGenDemo() {
  ...
  async function start() {
    ...
    setPhase("connecting");
    try {
      const token = await fetchSessionTokenForGabriel();
      ...
      if (!videoRef.current.id) videoRef.current.id = "gabriel-demo-stage";
  ...
  async function runSetup(teardown = false) {
    ...
    try {
      ...
      const r = await fetch(`/api/gabriel-tools-setup${teardown ? "?mode=teardown" : ""}`, {
        method: "POST",
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-border/40 bg-background/70 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-[1280px] items-center justify-between px-5 md:px-8">
          ...
          <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
            Lead-gen demo · Gabriel × Notion CRM
          </div>
          <div className="flex w-24 justify-end">{isSignedIn ? <UserButton /> : <Link to="/login?redirect=/demo/leadgen" className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground hover:text-foreground">Sign in</Link>}</div>
        </div>
      </header>
        ...
        <aside className="flex flex-col gap-5">
          <section className="rounded-2xl border border-border/60 p-4">
            <div className="flex items-center justify-between">
              <h2 className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Gabriel's actions</h2>
              <Wrench className="h-3.5 w-3.5 text-muted-foreground" />
            </div>
            <ul className="mt-3 flex max-h-[220px] flex-col gap-2 overflow-y-auto">
              {toolEvents.length === 0 ? (
                <li className="text-[12px] text-muted-foreground">Tool calls will appear here live as Gabriel works.</li>
              ) : (
                toolEvents.map((t) => (
